    ...
    <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
     ...
     <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
          <a class="dropdown-item" href="#"> One Hour Statistics</a>
          <a class="dropdown-item" href="#"> Home Statistics</a>
...
<div class="table-responsive">
  <table class="table" id="table">
    <tr><th colspan="12">Accepts</th></tr>
      ...
      <th>Reason</th> <th>Print</th></tr>
       <?PHP
            include 'connect.php';
            $query = "SELECT * FROM `accepts` ORDER BY `sno` DESC";
            $sql = mysqli_query($conn, $query);
              while($row = mysqli_fetch_array($sql))
              echo "<tr><td>".$row['sno']."</td><td>".$row['oneh/home']."</td><td>".$row['studentname']."</td><td>".$row['branch']."</td><td>".$row['roll']."</td><td>".$row['studying']."</td><td>".$row['leaving']."</td><td>".$row['arraival']."</td><td>".$row['place']."</td><td>".$row['phone']."</td><td>".$row['reason']."</td><td><a href='print.php?q=".$row['sno']."'target='blank'><b>PRINT</b></a></tr>";
            ?>
